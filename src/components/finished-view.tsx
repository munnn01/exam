...
interface FinishedViewProps {
  ...
  result: { score: number; total: number; violations: number };
  onHome: () => void;
}

export function FinishedView({ student, result, onHome }: FinishedViewProps) {
        ...
        <div className="panel overflow-hidden text-center">
          ...
          <div className="p-6 md:p-9">
            <div className="grid gap-3 sm:grid-cols-3">
              <ResultCard label="Điểm demo" value={`${result.score}/${result.total}`} note={`${percent}% câu đúng`} />
            ...
            <button className="primary-button mx-auto mt-7" onClick={onHome}><Home className="h-4 w-4" /> Về trang đăng nhập</button>
          </div>
        </div>
      </div>
    </main>
