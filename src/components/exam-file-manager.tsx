...
import { Archive, CalendarDays, Database, FileClock, FileLock2, FileText, HardDrive, LockKeyhole, RefreshCw, Settings2, Trash2, UnlockKeyhole, X } from "lucide-react";
...
function DeliverySettingsModal({ exam, onClose, onSave }: { exam: GeneratedExam; onClose: () => void; onSave: (assignedAt: string, maxAttempts: number) => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><form className="modal-card" onSubmit={async (event) => { event.preventDefault(); setBusy(true); setError(""); try { await onSave(new Date(assignedAt).toISOString(), maxAttempts); } catch (cause) { setError(cause instanceof Error ? cause.message : "Không thể cập nhật cấu hình."); setBusy(false); } }}><div className="flex items-start justify-between gap-4"><div><div className="text-xs font-bold text-teal-700">{exam.code}</div><h2 className="mt-1 text-2xl font-bold tracking-[-.04em]">Ngày giao và số lượt làm</h2><p className="mt-2 text-xs text-slate-500">{exam.title}</p></div><button type="button" className="icon-button" onClick={onClose}><X className="h-5 w-5" /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label><span className="form-label">Ngày và giờ giao</span><input className="form-input" type="datetime-local" required value={assignedAt} onChange={(event) => setAssignedAt(event.target.value)} /></label><label><span className="form-label">Lượt tối đa / sinh viên</span><input className="form-input" type="number" min={1} max={20} required value={maxAttempts} onChange={(event) => setMaxAttempts(Number(event.target.value))} /></label></div><div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600">Sinh viên có lượt đang làm sẽ tiếp tục đúng lượt đó. Giảm giới hạn xuống thấp hơn số lượt đã sử dụng sẽ bị từ chối.</div>{error && <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-700">{error}</div>}<div className="mt-7 flex justify-end gap-2"><button type="button" className="secondary-button" onClick={onClose}>Hủy</button><button className="primary-button" disabled={busy} type="submit">{busy ? "Đang lưu..." : "Lưu cấu hình"}</button></div></form></div>;
}
